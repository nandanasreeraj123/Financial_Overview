    ...
    if "df" in st.session_state and not st.session_state.df.empty:
        ...
        with dashboard_tab:
            ...
            col1.markdown(
                (
                    ...
                    f"<div class='metric-value'>€{income:,.2f}</div>"
                    f"<div class='metric-label'><i class='fa-solid fa-wallet'></i> "
                    f"Avg Monthly Income</div></div>"
                ),
                unsafe_allow_html=True,
            )
            ...
            col2.markdown(
                (
                    f"<div class='metric-card'>"
                    f"<div class='metric-value'>€{expenses:,.2f}</div>"
                    f"<div class='metric-label'><i class='fa-solid fa-credit-card'></i> "
                    f"Expenses</div></div>"
                ),
                unsafe_allow_html=True,
            )
            ...
            col3.markdown(
                (
                    f"<div class='metric-card'>"
                    f"<div class='metric-value'>€{savings:,.2f}</div>"
                    f"<div class='metric-label'><i class='fa-solid fa-piggy-bank'></i> "
                    f"Savings</div></div>"
                ),
                unsafe_allow_html=True,
            )
            ...
            col4.markdown(
                (
                    f"<div class='metric-card'>"
                    f"<div class='metric-value'>{savings_rate:.1f}%</div>"
                    f"<div class='metric-label'><i class='fa-solid fa-percentage'></i> "
                    f"Savings Rate</div></div>"
                ),
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            (
                ...
                'Background video: <a href="https://www.vecteezy.com/free-videos/background" '
                'target="_blank">Vecteezy Free Background Videos</a>'
                "</div>"
            ),
            unsafe_allow_html=True,
        )
